    ...
    <div class="mt-10 sm:mt-0">
      <div class="md:grid md:grid-cols-3 md:gap-6">
        ...
        <div class="mt-5 md:mt-0 md:col-span-2">
          <form action="{{ route('tecnicas.update', $tecnica) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <div class="col-span-6 sm:col-span-2">
                    <label for="first-name" class="block text-sm font-medium text-gray-700">Actual
                      Encabezado</label>
                    @foreach ($li as $l)
                    @if ($l->tecnica_p == $tecnicas->first_cod_tec)
                  ...
                  <div class="col-span-6 sm:col-span-2">
                    ...
                    <select id="id_tecnica" name="id_tecnica" required autocomplete="country" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                      ...
                      <option class="option" id="col{{ $enc->first_cod_tec }}" value="{{ $enc->first_cod_tec }}">{{$enc->title_tec}}</option>
                      @endforeach
                      @foreach ($tecnicaP as $tec)
                      @foreach($li as $l) @if($l->tecnica_p == $tec->first_cod_tec)<option class="option" id="col{{ $tec->first_cod_tec }}" selected="selected" value="{{ $tec->first_cod_tec }}">{{$l->title_tec}}</option> @endif @endforeach
                      @endforeach
                    </select>
                  </div>
                  ...
                  <div class="col-span-6 sm:col-span-2">
                    ...
                    <input type="text" name="ind_cod_tec" value="{{ $tecnicas->ind_cod_tec }}" id="ind_cod_tec" autocomplete="family-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>

                  <div class="col-span-6 sm:col-span-2">
                    <label for="last-name" class="block text-sm font-medium text-gray-700">Título</label>
                    <input type="text" name="title_tec" id="title_tec" value="{{ $tecnicas->title_tec }}" autocomplete="family-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>
                  ...
                  <div class="col-span-6 sm:col-span-2">
                    <div class="flex text-sm text-gray-600">
                      <label class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                        @if ($tecnicas->image_tec != null)<img id="imagenSeleccionada" src="/image/{{ $tecnicas->image_tec }}" width="150" alt="" /> @else @endif
                    ...
                    <button class="btn btn-danger btn-sm" href=""onclick="imageDelete({{$tecnicas->id}})">Eliminar Imagen</button>
                  </div>
                </div>
              </div>
